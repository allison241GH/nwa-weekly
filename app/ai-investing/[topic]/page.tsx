...
export async function generateStaticParams() {
  return AI_TOPIC_CATALOG.map((t) => ({ topic: t.slug }));
}
...
export default async function AiTopicPage({
  ...
  const meta = getAiTopicBySlug(topic);
  if (!meta) notFound();
  const lessons = getAiLessonsByTopic(topic);
  ...
  return (
    <div className="max-w-3xl mx-auto px-5 py-10">
      ...
      <header className="mb-8 pb-6 border-b border-slate-200">
        ...
        <h1 className="text-3xl font-bold text-[#6d28d9] mb-2">{meta.title}</h1>
      ...
      {lessons.length === 0 ? (
        <div className="rounded-xl border border-dashed border-slate-300 p-8 bg-slate-50">
          ...
          <p className="text-slate-700">
            ...
            here as they&rsquo;re taught in upcoming weekly runs.
          </p>
        </div>
      ) : (
        <section>
